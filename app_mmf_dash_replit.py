...
app.layout = dbc.Container([
    ...
    # INPUT 
    dbc.Row([
        dbc.Col([html.Label("Q (n° cave)"),       dcc.Input(id="input-q", type="number", value=27, min=2)], width=2),
        dbc.Col([html.Label("p (coppie)"),        dcc.Input(id="input-p", type="number", value=2, min=1)], width=2),
        dbc.Col([html.Label("m (fasi)"),          dcc.Input(id="input-m", type="number", value=3, min=1)], width=2),
        dbc.Col([html.Label("r (racc. passo)"),   dcc.Input(id="input-r", type="number", value=1, min=0)], width=2),
        dbc.Col([html.Label("Amp totale (A)"),    dcc.Input(id="input-ampfili", type="number", value=1, min=0)], width=2),
    ], className="mb-3"),
    dbc.Row([
        dbc.Col([html.Label("karm (n° arm.)"),    dcc.Input(id="input-karm", type="number", value=35, min=1)], width=2),
        dbc.Col([html.Label("nsamp (campioni)"),  dcc.Input(id="input-nsamp", type="number", value=720, min=1)], width=2),
        dbc.Col([html.Label("Δθ (°)"),           dcc.Input(id="input-step_deg", type="number", value=5.0, step=0.1, min=0.1)], width=2),
        dbc.Col([html.Label("N harm vis."),       dcc.Input(id="input-dis-harm", type="number", value=5, min=1)], width=2),
        dbc.Col([html.Label("N harm Σ"),          dcc.Input(id="input-cum-N", type="number", value=5, min=1)], width=2),
        dbc.Col([html.Label("soglia amp."),       dcc.Input(id="input-thresh", type="number", value=1e-3, step=1e-4, min=0)], width=2),
    ], className="mb-4"),
    # BOTTONI
    dbc.Row([
        ...
        dbc.Col(dbc.Button("matr. avv.",    id="btn-download-nc", color="info",    style=common_btn, disabled=True), width="auto"),
        dbc.Col(dcc.Input(id="input-nc-fn", type="text", value="Nc.tsv"), width="auto"),
        dbc.Col(dbc.Button("spettro arm.",  id="btn-download-harm", color="info", style=common_btn, disabled=True), width="auto"),
    ...
    # Dove appaiono grafico e animazione
    dcc.Graph(id="plot-fmm"),
    html.Img(id="anim-img", src="/animation.gif", style={"width":"100%", "margin-top":"1rem"}),
...
def generate_data(nc, Q, p, m, r, karm, nsamp, step_deg, ampfili):
    ...
    for idx in range(len(omega)):
        # estraggo solo la f.m.m. a gradini
        img = np.zeros((400,800,3), dtype=np.uint8)  # placeholder: qui crei il grafico come immagine
        # … qui si possono generare le immagini con matplotlib o PIL …
        frames.append(img)
    imageio.mimsave(ANIM_PATH, frames, fps= int(1000/400) )
    # bust-cache: aggiungo timestamp
    src = f"/animation.gif?{int(time.time())}"

    data = {
        "Q":Q, "p":p, "m":m, "r":r,
        "ampfili":ampfili, "karm":karm, "nsamp":nsamp,
        "theta":theta.tolist(),
        "mmf": mmf.tolist(),
        "Nc": Nc.tolist(),
    ...
    # abilito download e disabilito l’auto-animazione (si parte con Play)
    return data, False, False, True, src
...
def control_interval(n_gen, n_play, n_stop, disabled):
    ...
    if trig == "btn-generate":
        # appena genero: interval OFF, Play ON, Stop OFF
...
# route per servire la GIF
@server.route('/animation.gif')
def serve_animation():
    return send_file(ANIM_PATH, mimetype='image/gif')
